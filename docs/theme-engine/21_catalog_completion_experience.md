# Catalog Completion Experience

Version: 1.0  
Status: Binding product decision  
Category: Theme Engine Foundation, Phase 1C

## Final Data Decisions

`scope` and `origin` are persistent metadata of `AssetCatalogEntry`.
...
Normal user import may create only `theme` or `personal` entries.  
`core` is reserved for built-in, read-only Cosmos assets.
...
Normal file import always uses `imported`.

Automatically generated thumbnails and detail previews are derived catalog resources, not `VisualAsset` records.

An explicitly supplied thumbnail, preview or layer asset is imported and referenced as its own `VisualAsset`.
