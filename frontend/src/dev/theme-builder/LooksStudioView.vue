<template>
  ...
  >
    <section v-if="phase !== 'success'" class="looks-studio-state" :aria-live="phase === 'loading' ? 'polite' : undefined">
      ...
      <template v-else-if="phase === 'error'"><h1 class="builder-serif">Looks Studio unavailable</h1><p role="alert">{{ loadError }}</p><div class="looks-studio-state__actions"><button type="button" @click="loadProject">Try again</button><RouterLink :to="{ name: 'theme-builder' }">Back to Theme Board</RouterLink></div></template>
      <template v-else-if="phase === 'missing-template'"><h1 class="builder-serif">Missing Target Template</h1><p role="alert">{{ commandError }}</p><div class="looks-studio-state__actions"><RouterLink :to="{ name: 'theme-builder', query: { builderProjectId: projectId || undefined } }">Back to Theme Board</RouterLink></div></template>
      <template v-else>
    ...
    <div v-else-if="resolved && snapshot" class="looks-studio-workspace" data-testid="looks-studio-view">
      ...
      <button class="looks-studio-workspace__duplicate" type="button" :disabled="snapshot.dirty" @click="duplicateSkin">Duplicate Look</button>
      ...
      <div class="looks-studio-workspace__next"><span>{{ snapshot.dirty ? 'Save this Look before previewing it in context.' : 'Look saved and ready for an isolated preview.' }}</span><RouterLink :class="{ 'looks-studio-workspace__next--disabled': snapshot.dirty }" :aria-disabled="snapshot.dirty" :tabindex="snapshot.dirty ? -1 : 0" :to="snapshot.dirty ? {} : { name: 'theme-builder-showcase', query: { builderProjectId: projectId, skinId: resolved.skin.skinId } }" @click="snapshot.dirty && $event.preventDefault()">Preview in Showcase →</RouterLink></div>
    </div>
...
const corePreviewSkin = computed(() => {
  ...
  return [
    ...coreCosmosMapSkinPack.skins,
    ...coreCosmosGraphSkinPack.skins,
    ...coreDefaultBaseSkinPack.skins,
    ...coreWorkspaceUiSkinPack.skins,
...
const corePreviewFill = computed(() => {
  const value = coreTokenValue("cosmos.node.primary")
    ...
    ?? coreTokenValue("cosmos.workspace.background")
    ?? coreTokenValue("cosmos.window.surface");
...
const corePreviewStroke = computed(() => {
  const value = coreTokenValue("cosmos.node.accent")
    ...
    ?? coreTokenValue("cosmos.workspace.accent")
    ?? coreTokenValue("cosmos.window.accent");
...
const corePreviewOpacity = computed(() => {
  const value = coreTokenValue("cosmos.connection.opacity")
    ...
    ?? coreTokenValue("cosmos.workspace.opacity")
    ?? coreTokenValue("cosmos.window.opacity");
...
function duplicateSkin(): void {
  if (!session || !resolved.value || snapshot.value?.dirty) return;
  commandError.value = "";
  try {
    const before = new Set(session.snapshot.project.artifacts.skinPacks.flatMap((pack) => pack.skins.map((skin) => skin.skinId)));
    session.execute({ type: "duplicate-skin-draft", skinId: resolved.value.skin.skinId });
    syncSnapshot();
    const duplicate = session.snapshot.project.artifacts.skinPacks.flatMap((pack) => pack.skins).find((skin) => !before.has(skin.skinId));
    if (duplicate) void router.replace({ name: "theme-builder-looks", query: { ...route.query, builderProjectId: projectId.value, skinId: duplicate.skinId } });
  } catch (error) { commandError.value = error instanceof Error ? error.message : "Could not duplicate this Look."; }
}
