<template>
  <ThemeBuilderShell class="showcase-shell" active-studio="showcase" studio-label="Showcase" :builder-project-id="projectId">
    <section v-if="phase!=='success'" class="showcase-state"><h1 class="builder-serif">{{ phase==='loading'?'Preparing draft preview…':phase==='error'?'Preview unavailable':'No Builder Project selected' }}</h1><p>{{ phase==='error'?loadError:'Open a Theme Builder Project first.' }}</p><RouterLink v-if="phase!=='loading'" :to="{name:'theme-builder'}">Back to Theme Board</RouterLink></section>
    <section v-else-if="snapshot" class="showcase-v1" data-testid="showcase-view">
      ...
      <div v-if="resolved" class="showcase-v1__preview">
        ...
        />
        <aside><span>Previewing Look</span><h2 class="builder-serif">{{ resolved.skin.displayName }}</h2><p>This is an isolated Builder Draft preview. It does not mutate ThemeRuntime or the active installed Theme.</p><div class="showcase-v1__actions"><RouterLink :to="{name:'theme-builder-looks',query:{builderProjectId:projectId,skinId:resolved.skin.skinId}}">Open in Looks Studio</RouterLink><RouterLink class="showcase-v1__next" :to="{name:'theme-builder-release',query:{builderProjectId:projectId}}">Continue to Release</RouterLink></div></aside>
      </div>
...
<style scoped>
.showcase-shell{grid-template-columns:84px minmax(0,1fr) 0}.showcase-shell :deep(.builder-shell__context){display:none}.showcase-v1{display:grid;width:100%;height:100%;padding:32px 34px 36px;box-sizing:border-box;grid-template-rows:auto minmax(0,1fr);gap:24px;background:radial-gradient(circle at 70% 26%,rgba(90,126,158,.12),transparent 34%),radial-gradient(circle at 18% 80%,rgba(126,103,77,.05),transparent 30%),#080d11}.showcase-v1__header{display:flex;align-items:end;justify-content:space-between;gap:24px}.showcase-v1__header span,.showcase-v1__header p{color:var(--builder-muted);font-size:.72rem}.showcase-v1__header h1{margin:5px 0;font-size:2.55rem;line-height:1;letter-spacing:-.025em}.showcase-v1__header p{margin:0}.showcase-v1__stats{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:8px}.showcase-v1__stats span{padding:6px 9px;border:1px solid var(--builder-border);border-radius:999px}.showcase-v1__preview{display:grid;min-height:0;grid-template-columns:minmax(0,1fr) 290px;overflow:hidden;border:1px solid rgba(154,174,191,.16);border-radius:var(--builder-radius-hero);background:rgba(9,14,18,.7);box-shadow:0 28px 70px rgba(0,0,0,.28),inset 0 1px rgba(255,255,255,.02)}.showcase-v1__preview>:deep(.looks-studio-canvas){min-height:0}.showcase-v1__preview aside{display:grid;padding:28px;align-content:center;border-left:1px solid var(--builder-border);gap:10px}.showcase-v1__preview aside span{color:var(--builder-faint);font-size:.65rem;text-transform:uppercase}.showcase-v1__preview aside h2,.showcase-v1__preview aside p{margin:0}.showcase-v1__preview aside p{color:var(--builder-muted);font-size:.72rem;line-height:1.5}.showcase-v1__preview a,.showcase-v1__empty a,.showcase-state a{width:fit-content;margin-top:8px;padding:9px 12px;border:1px solid var(--builder-border-strong);border-radius:7px;background:rgba(120,149,177,.12);color:var(--builder-text);text-decoration:none}.showcase-v1__actions{display:flex;flex-wrap:wrap;gap:8px}.showcase-v1__actions a{margin-top:8px}.showcase-v1__actions .showcase-v1__next{border-color:rgba(159,187,211,.5);background:rgba(120,149,177,.2)}.showcase-v1__empty,.showcase-state{display:grid;min-height:100%;place-content:center;justify-items:start;gap:10px}.showcase-v1__empty h2,.showcase-v1__empty p,.showcase-state h1,.showcase-state p{margin:0}.showcase-v1__empty p,.showcase-state p{max-width:520px;color:var(--builder-muted)}
</style>
