...
function openValidationTarget(item:ReleaseValidationItem):void{if(!item.target)return;const builderProjectId=projectId.value||undefined;if(item.target==="library"){void router.push({name:"theme-builder-assets",query:{returnBuilderProjectId:builderProjectId,returnBuilderRoute:"theme-builder-release"}});return}const name=item.target==="board"?"theme-builder":"theme-builder-looks";void router.push({name,query:{builderProjectId}});}
