<template>
  ...
  >
    <section v-if="phase!=='success'" class="release-state"><h1 class="builder-serif">{{ phase==='loading'?'Loading release data…':phase==='error'?'Release unavailable':'No Builder Project selected' }}</h1><p>{{ phase==='error'?loadError:'Open a Theme Builder Project first.' }}</p></section>
...
<style scoped>
.release-shell{grid-template-columns:72px minmax(0,1fr) 0}.release-shell :deep(.builder-shell__canvas){overflow:hidden}.release-shell :deep(.builder-shell__context){display:none}.release-studio{display:grid;width:100%;height:100%;min-width:0;min-height:0;padding:14px 16px 16px;box-sizing:border-box;grid-template-rows:minmax(250px,1.45fr) minmax(170px,1fr) minmax(95px,.55fr) minmax(112px,.65fr);gap:10px}.release-studio__section{display:grid;min-width:0;min-height:0;grid-template-rows:24px minmax(0,1fr);gap:4px}.release-studio__section h2{margin:0 0 0 3px;align-self:center;color:var(--builder-text);font-size:1.04rem;font-weight:400;line-height:1}.release-state{display:grid;min-height:100%;place-content:center;gap:10px}.release-state h1,.release-state p{margin:0}.release-state p{color:var(--builder-muted)}.release-export-error{position:absolute;right:24px;bottom:18px;margin:0;padding:8px 11px;border:1px solid rgba(198,155,118,.4);border-radius:7px;background:#12171b;color:#c69b76;font-size:.7rem}
</style>
